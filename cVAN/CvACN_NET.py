...
from Transformer.model.Transformer import TransformerBlock, TokenAndPositionEmbedding


def conv_block(input_tensor, num_output, stride):
    filters_1, filters_2 = num_output

    x = keras.layers.SeparableConv2D(filters_1, 3, strides=stride, padding='same',
                                     kernel_regularizer=keras.regularizers.l1_l2(0.01))(input_tensor)
    x = keras.layers.BatchNormalization()(x)
    x = keras.layers.Activation('relu')(x)

    x = keras.layers.SeparableConv2D(filters_2, 3, strides=(1, 1), padding='same',
                                     kernel_regularizer=keras.regularizers.l1_l2(0.01))(x)
    x = keras.layers.BatchNormalization()(x)
    x = keras.layers.Activation('relu')(x)

    shortcut = keras.layers.SeparableConv2D(filters_2, 1, strides=stride,
                                            kernel_regularizer=keras.regularizers.l1_l2(0.01))(input_tensor)
...
def identity_block(input_tensor, num_output):
    filters_1, filters_2 = num_output

    x = keras.layers.SeparableConv2D(filters_1, 3, strides=(1, 1), padding='same',
                                     kernel_regularizer=keras.regularizers.l1_l2(0.01))(input_tensor)
    x = keras.layers.BatchNormalization()(x)
    x = keras.layers.Activation('relu')(x)
...
def residual_networks(input):
    ...
    # conv2_x
    x = conv_block(input_tensor=x, num_output=(64, 64), stride=(1, 1))
    x = identity_block(input_tensor=x, num_output=(64, 64))
    x = SCAtnn(x)
    scale_x1 = downsample(x)
    cross_x1 = cross_aliment(x)

    # conv3_x
    x = conv_block(input_tensor=x, num_output=(128, 128), stride=(2, 2))
    x = identity_block(input_tensor=x, num_output=(128, 128))
    x = SCAtnn(x)
    scale_x2 = downsample(x)
    cross_x2 = cross_aliment(x)

    # conv4_x
    x = conv_block(input_tensor=x, num_output=(256, 256), stride=(2, 2))
    x = identity_block(input_tensor=x, num_output=(256, 256))
    x = SCAtnn(x)
    coss_x3 = cross_aliment(x)
    x_skip3 = downsample(x)

    # conv5_x
    x = conv_block(input_tensor=x, num_output=(512, 512), stride=(2, 2))
    x = identity_block(input_tensor=x, num_output=(512, 512))
    x = SCAtnn(x)
...
def build_cVAN(vocab_size, maxlen, num_class, d_model, num_heads, num_layers, ff_dim):
    ...
    x, x_cross1, x_cross2, x_cross3 = residual_networks(input_2)
    ...
    model = keras.Model(inputs=[input_1, input_2], outputs=output_tensor)
...
# SCAtnn
def SCAtnn(x):
    context_channel = channel_only_attention(x)
    context_spatial = spatial_only_attention(x)
    out = keras.layers.MultiHeadAttention(num_heads=2, key_dim=64, )(x, context_spatial, context_channel)
    return out


def channel_only_attention(x):
