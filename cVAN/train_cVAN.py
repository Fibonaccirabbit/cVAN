...
from Transformer.cVAN.CvACN_NET import build_cVAN
from Transformer.cVAN.Utils.Utils import PrintScore, VariationCurve, process_batch_signal, ReadConfig
from Transformer.model.DataGenerator import kFoldGenerator
from Transformer.model.Utils import ConfusionMatrix
...
for i in range(10):
    print(128 * '_')
    print('Fold #', i)
    checkpoint_filepath = "model_cVAN_S1_" + str(i) + ".hdf5"
    ...
    x_train, y_train, x_test, y_test = DataGenerator.getFold(i)
    x_train_1 = np.array(process_batch_signal(x_train))
    x_test_1 = np.array(process_batch_signal(x_test))
    x_train = np.apply_along_axis(lambda x: np.interp(np.linspace(0, len(x), 500), np.arange(len(x)), x), axis=2, arr=x_train)
    ...
    history_fea = model.fit([x_train,x_train_1], y_train, batch_size=32, epochs=30,
                            validation_data=([x_test,x_test_1], y_test),
    ...
    if i == 0:
        fit_loss = np.array(history_fea.history['loss']) * Test_Fold_Num[i]
        fit_acc = np.array(history_fea.history['accuracy']) * Test_Fold_Num[i]
        fit_val_loss = np.array(history_fea.history['val_loss']) * Test_Fold_Num[i]
        fit_val_acc = np.array(history_fea.history['val_accuracy']) * Test_Fold_Num[i]
    else:
        fit_loss = fit_loss + np.array(history_fea.history['loss']) * Test_Fold_Num[i]
        fit_acc = fit_acc + np.array(history_fea.history['accuracy']) * Test_Fold_Num[i]
        fit_val_loss = fit_val_loss + np.array(history_fea.history['val_loss']) * Test_Fold_Num[i]
        fit_val_acc = fit_val_acc + np.array(history_fea.history['val_accuracy']) * Test_Fold_Num[i]
